...
- Windowed processing for long videos in `scripts/watch-mimo.sh` — past 10 min (configurable via `$WATCH_WINDOW_SECONDS`), the harness probes duration (new `watch.py --probe`) and processes the video in 10-minute windows, each with its own dense per-shot budget, appending per-window findings to the report as durable working memory. Avoids squeezing an hour-long video into a single 100-frame "sparse scan." Explicit `--start`/`--end` ranges skip windowing.
- `--cleanup` flag on `scripts/watch-mimo.sh` — removes the extracted-frames working dir after the report is written (the cached video is kept).
- `--scene-change` flag on `scripts/watch.py` — forces scene-change sampling even inside a focused range (used by windowed passes); its uniform fallback now uses the denser focused budget when a range is set.
- `--probe` flag on `scripts/watch.py` — prints `{duration_seconds, title}` as JSON and exits without extracting frames (used to plan windows).
- `/watch-mimo <url-or-path> [question]` slash command (`commands/watch-mimo.md`) — invokes the MiMo harness and surfaces the generated report. The harness writes `watch-analysis.md` to the caller's working directory (override with `$WATCH_ANALYSIS_OUT`) rather than the plugin dir. Has a fallback: the launcher retries transient errors (rate-limit/overload, `$WATCH_MIMO_MAX_RETRIES`) and, if MiMo still fails, the command falls back to the standard `/watch` flow with Claude as the agent.
